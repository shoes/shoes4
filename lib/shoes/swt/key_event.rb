class Shoes
  module Swt
    class Keyevent
      def initialize dsl, app, event_type, &blk
        @dsl = dsl
        @app = app
        @shell = app.shell
        @key_listener = KeyListener.new event_type, blk
    ...
    class KeyListener
      ...
      def initialize(event_type = :pressed, blk)
      ...
      def key_pressed(event)
        return unless @event_type == :pressed || @event_type == :all
        handle_key_event(event)
      end

      def key_released(event)
        return unless @event_type == :released || @event_type == :all
        handle_key_event(event)
      end
