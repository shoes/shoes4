...
Shoes.app title: "vJot", width: 400, height: 560, resizable: false do
  ...
  stack width: 400, margin: 20 do
    ...
    stack margin: 20 do
      caption "vJot"
      @title = edit_line @note[0], width: 300 do
        @note[0] = @title.text
        load_list
      end
      stack width: 1.0, height: 180, scroll: true do
        @list = para
      end
      @jot = edit_box @note[1], width: 300, height: 200, margin_bottom: 20 do
        @note[1] = @jot.text
      end
      para size: 5
    end
