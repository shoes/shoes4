...
# Not working yet... too many building blocks missing. Such as Shoes::Search which is in shoes3/lib/shoes/search.rb but it uses an old outdated gem, which has a fork but that also seems unmaintained for 3 years
# Also a couple of methods missing like Shoes.read_file although I'm uncertain if we really want to implement those.
module Shoes::Manual
  PARA_RE = /\s*?(\{{3}(?:.+?)\}{3})|\n\n/m
  CODE_RE = /\{{3}(?:\s*\#![^\n]+)?(.+?)\}{3}/m
  IMAGE_RE = /\!(\{([^}\n]+)\})?([^!\n]+\.\w+)\!/
  ...
  def sample_page
    folder = File.join Shoes::DIR, 'samples'
    h = {}
